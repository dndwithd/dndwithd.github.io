...
function filter_sort(population, fltsrt=[]) {
    ...
    for (indx in fltsrt) {
        ...
        switch(fltsrt[indx][1]) {
            case '>':
                for (each in oldpop) {
                    if (pop[oldpop[each]][fltsrt[indx][0]] > fltsrt[indx][2]) {
            ...
            case '+':
                ...
                for (each in oldpop) {
                    pos = 0;
                    for (item in newpop) {
                        if (pop[oldpop[each]][fltsrt[indx][0]] > pop[newpop[item]][fltsrt[indx][0]]) {
                            pos = item;
                            pos++;
                        };
                    };
                    //console.log("sort: " + ' ' + pos + ' ' + oldpop[each])
...
function filtsort(population) {
    ...
    var oldpop=[], newpop=[], indx=0;
    //document.getElementById("ts").innerHTML += "<br><br>filtsort: entry ";
    if (population.length < 1) {
        return [];
    } else {
    	for (indx = 0; indx < population.length; indx++) {
    		oldpop.push(indx);
    	};
    };
